...
router.post("/getstudentsonly", (req, res) => {
			...
			db.query(studentMappingSkills).then((result)=>{
				...
				if(result.rows[0].exists){
					db.query(updatedSkillsQuery)
						.then((result) =>{
							res.status(200).json({ "lessons":lessonsArray, "competencies":result.rows })
						}).catch((e) => res.status(500).json({ "message":e }));
				} else {
		...
		if(loginResult === undefined){
			...
			if ( isValid ){
				...
				db.query(tokenInsert , [newToken, userId, creationDate])
					.then((result) => res.status(200).json({ "message": userRole, "id":userId, "name":userName, "tokenMsg":"token created","token":newToken }))
